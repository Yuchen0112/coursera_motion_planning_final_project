...
class PathOptimizer:
    ...
    def optimize_spiral(self, xf, yf, tf):
        ...
        # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
        # ------------------------------------------------------------------
        res = scipy.optimize.minimize(fun=self.objective, x0=np.asarray(p0), method='L-BFGS-B', jac=self.objective_grad,
                                      bounds=bounds)
        # ------------------------------------------------------------------
    ...
    def thetaf(self, a, b, c, d, s):
        ...
        a, b, c, d, s = np.asarray(a), np.asarray(b), np.asarray(c), np.asarray(d), np.asarray(s)
